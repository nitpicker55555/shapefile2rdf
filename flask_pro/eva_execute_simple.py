...
def insert_data(each_data):
    ...
    if buffer_num:
        print(buffer_num[0])
        feedback = create_eva([subject_str, subject_name], [object_str, object_name], cursor, conn,
                              geo_relation=geo_relation, distance=int(buffer_num[0]))
    else:
...
for each_data in data_lines:
    ...
    result.append(
        {'query': each_data, 'total_token': tokens, 'time': total_time, 'sql': extract_sql(sql), 'statues': statues})
    print(result[-1])
    print(statues, 'statues')
    print(feedback, 'feedback')
    with open('first_result.jsonl', 'a', encoding='utf-8') as file:
        row_dict = {'query': each_data, 'total_token': tokens, 'time': total_time, 'sql': sql, 'statues': statues,
                    'check_query': feedback}

...
def repair(input_path,output_path=None):
    if output_path==None:
        output_path=input_path.replace(".ttl","repaired.ttl")
...
def crs_transfer(input_file_path,output_file_path=None):
    ...
    if output_file_path==None:
        output_file_path=os.path.dirname(input_file_path)+"\\"+os.path.basename(input_file_path).replace(".ttl","4326.ttl")
    ...
    # 运行脚本
    # input_file_path = r'C:\Users\Morning\Desktop\hiwi\ttl_query\ttl_file\modified_Moore_Bayern.ttl'  # 替换为你的输入文件路径
    # output_file_path = r'C:\Users\Morning\Desktop\hiwi\ttl_query\ttl_file\modified_Moore_Bayern_4326.ttl'  # 替换为你的输出文件路径

...
$(document).ready(function() {
    ...
    chatBtn.click(function() {
        ...
        // 保存api key与对话数据
        var data = {
            "apiKey" : "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", // 这里填写固定 apiKey
        }
        ...
        xhr.open('POST', '123.php', true);
        xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        xhr.send('input-text=' + encodeURIComponent(message));
        ...
        // 收到回复前让按钮不可点击
        chatBtn.innerText="正在生成答案..."
        chatBtn.innerHTML="正在生成答案..."
...
stream:true
            ...
            success: function(res) {
                console.log(res)
                const resp = res["choices"][0].text;

                addMessage(resp.content,"chatgpt.png");
                // 收到回复，让按钮可点击
                chatBtn.attr('disabled',false)
                chatBtn.innerText="提问"
                chatBtn.innerHTML="提问"
                // 重新绑定键盘事件
                chatInput.on("keydown",handleEnter);
                // 将回复添加到数组
                messages.push(resp)
